...
ShpyOrt_api = Flask(__name__)
ShpyOrt_api.config.from_object('config')
...
@ShpyOrt_api.route('/api/v1/shorten' , methods= ['POST'])
		...
		if 'url' in request.args :
			...
			if url_check(og_url) is True:
				...
				if (check_fetch is None):
					...
					short_url = ShpyOrt_host+token_string 
					long_url = og_url
					data = jsonify({
						'long_url' : og_url,
						'short_url' : short_url,
...
@ShpyOrt_api.route('/api/v1/expand' , methods= ['GET'])
...
if __name__ == '__main__':
	...
	ShpyOrt_api.run(host='127.0.0.1', port=8000 )
