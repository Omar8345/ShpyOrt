...
def analytics(short_url):
	...
	return render_template("data.html" , host = ShpyOrt_host,info = info_fetch ,counter = counter_fetch ,\
		...
		if og_url != '':
			if url_check(og_url) == True:
				...
				if (check_fetch is None):
					...
					conn.close()
					e = ''
					return render_template('index.html' ,ShpyOrt_url = ShpyOrt_host+token_string , error = e )
				else:
					e = "The Custom suffix already exists . Please use another suffix or leave it blank for random suffix."
					return render_template('index.html' ,table = result_all_fetch, host = ShpyOrt_host,error = e)
			else:
...
def search():
	...
	if s_tag == "":
		...
		conn.close()
		return render_template('search.html' , host = ShpyOrt_host , search_tag = s_tag , table = search_tag_fetch )
...
if __name__ == '__main__':
	...
	# Logging handler
	handler = RotatingFileHandler('ShpyOrt.log' , maxBytes=100000 , backupCount = 3)
